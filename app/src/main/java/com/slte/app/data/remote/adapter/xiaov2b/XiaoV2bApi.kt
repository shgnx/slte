...
interface XiaoV2bUserRetrofit {
    ...
    // 吊销请求显式携带 Authorization 头：登出流程先清空本地会话，
    // 此时拦截器已取不到 token，否则吊销会因无认证头而静默失败
    ...
    /**
     * 订阅 Clash YAML（文本响应）：路径经构建期 SLTE_SUBSCRIBE_PATH 注入，与内核 profile source 一致。
     * 携带 X-SLTE-No-Failover 头，请求不参与容灾切换。
     */
    @GET
    @Headers(
