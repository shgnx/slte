 ...
 *
 * 类型族不同故不与 Xboard 适配器合并（见 [com.slte.app.data.remote.adapter.xboard.XboardAuthApi]）；
 * 共享执行外壳已外提到 AdapterExecute，鉴权由 OkHttp 拦截器统一注入。
 */
class XiaoV2bAuthApi(
    ...
    override suspend fun revokeActiveSessions(authData: String) {
        ...
        // 会话无法按 authData 可靠匹配，遍历全部会话逐个吊销，保证登出后服务端 JWT 失效
